...
MatrixPosture::MatrixPosture(const int16_t* matrixPtr, uint8_t rows, uint8_t cols)
...
float MatrixPosture::deltaAt(uint16_t idx) const {
    if (!_hasBaseline) return 0.f;
    if (!_matrix)     return 0.f;

    int raw = _matrix[idx];
    if (raw == INVALID_INT) return 0.f;
...
float MatrixPosture::zoneDeltaSum(MatrixZoneId id) const {
  const MatrixZone& z = _zones.zone(id);
  if (!_hasBaseline || !_matrix) return 0.f;

  static constexpr float CELL_EPS = 15.0f;  // unter ~15 ADC-Punkte: als Rauschen ignorieren
  ...
  for (uint8_t i = 0; i < z.count; ++i) {
    ...
    // sehr kleine Änderungen ignorieren
    if (d > -CELL_EPS && d < CELL_EPS) continue;

    // nur zusätzliche Last berücksichtigen
    if (d > 0.f) {
      sum += d;
    }
  }
...
MatrixTags MatrixPosture::computeTags() const {
  ...
  static constexpr float OCC_MIN = 8000.0f; // dein Wert, der gut funktioniert
  if (fullDelta > OCC_MIN) {
    tags.occupied = true;
  } else {
    tags.occupied = false;
    tags.neutral  = false;   // „sitzend neutral“ macht bei unbesetzt keinen Sinn
    return tags;             // niemand sitzt drauf → fertig
  }

  // Ab hier wissen wir: jemand sitzt drauf
  // Default: erstmal neutral an, dann ggf. bei starken Leans wieder aus.
  tags.neutral = true;
...
MatrixTags MatrixPosture::computeTagsBaseline() const {
    ...
    if (occThreshold < OCC_MIN_ABS) occThreshold = OCC_MIN_ABS;
    ...
    //    Achtung: LeftHalf/RightHalf sind im Code vertauscht zur physischen Seite,
    //    darum wie gehabt: RightHalf = physikalisch links, LeftHalf = physikalisch rechts.
    float leftDelta  = zoneDeltaSum(MatrixZoneId::RightHalf); // physikalisch links
    float rightDelta = zoneDeltaSum(MatrixZoneId::LeftHalf);  // physikalisch rechts
    ...
    // Mindest-Gesamt-Delta, damit kleinste Bewegungen nicht gleich als Lean zählen
    ...
    // Milde Leans lassen neutral an; starke Leans haben es oben bereits ausgeschaltet.
