...
struct ToF {
  ...
  bool valid = false; // bool on if the init() worked
...
public:
  ...
  static constexpr uint8_t MAX_L1X = MAX_TOF;//Pools with Sensor instances - could be reduced to actuall amounts of sensors if clear
  ToFManager();

  bool add(const ToF& tof); //add a tof to the list of all ToFs
  void init(); //logically seperates between two types of sensors and uses specific init for each sensor
...
private:
  ToF _tofs[MAX_TOF];
  uint8_t _count = 0;
  VL53L0X  _l0xPool[MAX_L0X];  uint8_t _l0xN = 0;
  VL53L1X  _l1xPool[MAX_L1X];  uint8_t _l1xN = 0;
